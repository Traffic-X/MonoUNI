...
def laplacian_aleatoric_uncertainty_loss(input, target, log_variance,bili=1, reduction='mean'):
    '''
    References:
    ...
    loss =bili *  1.4142 * torch.exp(-0.5*log_variance) * torch.abs(input - target) + 0.5*log_variance
    if reduction=='none':
        return loss
